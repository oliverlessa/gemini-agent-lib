...
async function vertexSearchRetrieverImplementation(args) {
    console.log(`Chamada para vertex_ai_search com args:`, args);
    ...
    try {
        console.log("Configuração da requisição:", JSON.stringify({
            apiEndpoint,
            servingConfig: name,
            query: query,
            pageSize: maxResults
        ...
        // Executar busca
        console.log("Executando busca no Vertex AI Search...");
        const response = await client.search(request, {
            autoPaginate: false
        });
        
        console.log("Resposta recebida do Vertex AI Search");
        
        // Processar resultados
        const results = response[0].results || [];
        console.log(`Número de resultados encontrados: ${results.length}`);
...
const vertexSearchRetrieverTool = ToolBuilder.createTool("search", "Realiza buscas usando o Google Vertex AI Search")
