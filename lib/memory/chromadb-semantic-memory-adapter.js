...
class ChromaDBMemoryAdapter extends SemanticMemory {
   ...
   * @param {VertexAIEmbeddingFunction} config.embeddingFunction - Instância da função de embedding a ser usada (VertexAIEmbeddingFunction).
  ...
  constructor({ path, collectionName, embeddingFunction, collectionMetadata }) {
    ...
    if (!(embeddingFunction instanceof VertexAIEmbeddingFunction)) {
      // Garante que estamos usando nossa função de embedding customizada
      throw new Error("A embeddingFunction fornecida deve ser uma instância de VertexAIEmbeddingFunction.");
    }
  ...
  async init() {
    try {
      ...
      // O ChromaDB JS espera uma *instância* de IEmbeddingFunction.
      // Precisamos adaptar nossa classe VertexAIEmbeddingFunction para a interface esperada pelo ChromaDB.
      const chromaEmbeddingFunction = {
        generate: async (texts) => {
          // Chama o método da nossa instância VertexAIEmbeddingFunction
      ...
      this.collection = await this.client.getOrCreateCollection({
        ...
        embeddingFunction: chromaEmbeddingFunction, // Passa a função adaptada
