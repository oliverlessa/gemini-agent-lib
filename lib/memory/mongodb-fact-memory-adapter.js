...
class MongoDBFactMemoryAdapter extends FactMemory {
    ...
    async initialize() {
        try {
            ...
            await this.factsCollection.createIndex({ context_id: 1, key: 1 }, { unique: true });
            console.log(`[MongoDBFactMemoryAdapter] Conectado ao MongoDB, database: '${this.dbName}', collection: '${this.collectionName}'`);
        } catch (error) {
    ...
    async close() {
        if (this.client) {
            try {
                ...
                console.log("[MongoDBFactMemoryAdapter] Conexão com o banco de dados MongoDB fechada.");
                this.client = null;
                this.factsCollection = null;
            } catch (error) {
