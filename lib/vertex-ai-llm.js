    ...
    async generateContent({ prompt, tools, context, history }) {
        try {
            ...
            if (this.mode === 'chat') {
                ...
                const chatResult = await this.chat.sendMessage(prompt);
                ...
                if (candidate.groundingMetadata && candidate.groundingMetadata.webSearchRetrievalResults) {
                    console.log("GroundingMetadata encontrado:", JSON.stringify(candidate.groundingMetadata, null, 2));
                    ...
                    if (fontes.length > 0) {
                        responseResult.text = `${responseResult.text}\n\n**Fontes:**\n${fontes.join('\n')}`;
                    }
                }
