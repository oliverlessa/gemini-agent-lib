...
class SQLiteConversationMemoryAdapter extends ConversationMemory {
    ...
    constructor({ dbConfig }) {
        if (!dbConfig || !dbConfig.dbPath) {
            throw new Error("SQLiteConversationMemoryAdapter requer 'dbConfig.dbPath'.");
        }
    ...
    async close() {
        return new Promise((resolve, reject) => {
            this.db.close((err) => {
                ...
                console.log("[SQLiteConversationMemoryAdapter] Conexão com o banco de dados SQLite fechada.");
                resolve();
            });
        });
    }
