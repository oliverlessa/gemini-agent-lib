...
class Agent {
    ...
    async executeTask() {
        try {
            ...
            // Verifica se o Google Search está habilitado e se há metadados de grounding
            if (this.enableGoogleSearch && response && response.candidates && 
                response.candidates[0] && response.candidates[0].groundingMetadata) {
                
                // Log temporário para verificar o objeto candidates[0]
                console.log("DEBUG - Objeto response.candidates[0]:", JSON.stringify(response.candidates[0], null, 2));
                
                const groundingMetadata = response.candidates[0].groundingMetadata;
                console.log("GroundingMetadata:", JSON.stringify(groundingMetadata, null, 2));
                
                // Extrai as fontes dos metadados de grounding
                let fontes = [];
                if (groundingMetadata && groundingMetadata.webSearchRetrievalResults) {
                    fontes = groundingMetadata.webSearchRetrievalResults.map((result, index) => {
                        return `[${index + 1}] ${result.uri}: ${result.title}`;
                    });
                }
                
                // Adiciona as fontes à resposta final
                const textoResposta = response.text || "Nenhuma resposta do LLM.";
                if (fontes.length > 0) {
