...
class AgentRegistry {
    ...
    registerSpecialist(role, config) {
        ...
        debug(`Especialista '${role}' registrado com sucesso`);
    ...
    unregisterSpecialist(role) {
        if (this.specialistAgentsConfig[role]) {
            ...
            delete this.llmInstances[role];
            delete this.agentInstances[role];
            debug(`Especialista '${role}' removido com sucesso`);
        } else {
    ...
    _getSpecialistLLM(role) {
        ...
        if (!this.llmInstances[llmKey]) {
            debug(`Criando nova instância LLM para: ${llmKey}`);
    ...
    getSpecialistAgent(role, forceNew = false) {
        ...
        if (forceNew || !this.agentInstances[role]) {
            ...
            debug(`Criando nova instância de ${AgentClass.name} para role: ${role}`);
    ...
    clearCache() {
        this.llmInstances = {};
        this.agentInstances = {};
        debug('Cache de instâncias limpo');
        return this;
    }
}
