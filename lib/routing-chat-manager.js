...
`;
            ...
            if (response && response._internalToolSignal) {
                ...
                if (signal._signal_type === "REQUEST_SUB_CONVERSATION") {
                    ...
                    if (specialistAgent && specialistAgent !== state.primaryAgent) {
                        ...
                        return response; // Retorna a resposta original do coordenador
                    }
                    // --- FIM DA NOVA LÓGICA ---
                } else if (signal._signal_type === "END_SUB_CONVERSATION") { // Este bloco permanece o mesmo
                    debug.routingChatManager(`Sinal END_SUB_CONVERSATION detectado. Finalizando sub-conversa.`);
            ...
            // Se não houve sinal ou o sinal foi tratado de outra forma (como END_SUB_CONVERSATION),
            // retorna a resposta original do agente que foi chamado.
            return response;
        } catch (error) {
