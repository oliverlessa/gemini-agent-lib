...
class ChatAgent extends Agent {
    ...
    constructor({
        ...
        // Configurar delegação para especialistas (se habilitada)
        let allTools = [...tools]; // Começa com as ferramentas fornecidas pelo usuário
        let finalContext = context; // Contexto que será passado para o construtor da classe pai
        ...
        if (enableSpecialistDelegation) {
            try {
                ...
                if (availableRoles.length === 0) {
                    ...
                    if (!context) {
                        finalContext = `Você é um ${role} com o objetivo de ${objective}.
...
6. NÃO invente especialistas ou use a ferramenta de delegação para tarefas simples.`;
                    ...
                    allTools.unshift(this._createDelegationTool(availableRoles));
