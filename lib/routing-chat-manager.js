...
`;
            ...
            const agentName = agentToProcess === state.primaryAgent ? 'agente primário' : `especialista: ${state.activeSpecialistRole}`;
            debug.routingChatManager(`Enviando mensagem para ${agentName}: "${messageForAgent.substring(0, 100)}..."`);
            const response = await agentToProcess.processUserMessage(messageForAgent);
            debug.routingChatManager(`Resposta recebida de ${agentName}: %o`, response); // Log da resposta completa
            ...
            if (response && response._internalToolSignal) {
                ...
                debug.routingChatManager(`Sinal interno detectado: %o`, signal); // Log do sinal detectado

                if (signal._signal_type === "REQUEST_SUB_CONVERSATION") {
                    debug.routingChatManager(`Processando sinal REQUEST_SUB_CONVERSATION. Detalhes: %o`, signal.details);
                    ...
                    const specialistAgent = state.activeAgent; // O agente ativo já foi atualizado por _startSubConversation
                    const originalUserMessage = message; // A mensagem original que iniciou este processMessage

                    if (specialistAgent && specialistAgent !== state.primaryAgent) {
                        debug.routingChatManager(`Enviando mensagem original "${originalUserMessage.substring(0, 100)}..." para o novo especialista ${state.activeSpecialistRole}`);
                        ...
                        debug.routingChatManager(`Resposta recebida do novo especialista ${state.activeSpecialistRole}: %o`, specialistResponse); // Log da resposta do especialista
                    ...
                    if (agentToProcess !== state.primaryAgent) {
                        debug.routingChatManager(`Processando sinal END_SUB_CONVERSATION do especialista ${state.activeSpecialistRole}. Detalhes: %o`, signal.details);

                        // Finaliza a sub-conversa e armazena o resultado pendente
                        await this._endSubConversation(state, signal.details); // _endSubConversation já loga a finalização
                        ...
                        state.pendingSpecialistResult = null; // Limpar imediatamente após obter
...
Agora, processe a mensagem original do usuário abaixo, que não foi respondida pelo especialista:
${message}
`;
                        debug.routingChatManager(`Enviando mensagem formatada para o agente primário: "${messageForPrimary.substring(0, 200)}..."`);
                        const primaryResponse = await state.primaryAgent.processUserMessage(messageForPrimary);
                        debug.routingChatManager(`Resposta recebida do agente primário após END_SUB_CONVERSATION: %o`, primaryResponse); // Log da resposta do primário
