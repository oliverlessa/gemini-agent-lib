...
class MongoDBSummaryMemoryAdapter extends SummaryMemory {
    ...
    async initialize() {
        try {
            ...
            // Criar índice para buscas eficientes por timestamp DESC
            await this.summariesCollection.createIndex({ context_id: 1, timestamp: -1 });
            console.log(`[MongoDBSummaryMemoryAdapter] Conectado ao MongoDB, database: '${this.dbName}', collection: '${this.collectionName}'`);
        } catch (error) {
    ...
    async close() {
        if (this.client) {
            try {
                ...
                console.log("[MongoDBSummaryMemoryAdapter] Conexão com o banco de dados MongoDB fechada.");
                this.client = null;
                this.summariesCollection = null;
            } catch (error) {
