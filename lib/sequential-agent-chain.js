const debug = require('./debug').create('gemini-agent-lib:orchestrator:sequential');
...
class SequentialAgentChain {
    ...
    async run(initialInput) {
        ...
        debug("--- Iniciando Cadeia Sequencial de Agentes ---");

        for (let i = 0; i < this.agents.length; i++) {
            const agent = this.agents[i];
            debug(`**Agente ${i + 1}: ${agent.role} - Iniciando tarefa... **`);
            ...
            try {
                ...
                debug(`**Agente ${i + 1}: ${agent.role} - Tarefa Concluída.**`);
                debug(`Saída do Agente ${i + 1}: %o`, output);
        ...
        debug("--- Cadeia Sequencial de Agentes Concluída ---");
