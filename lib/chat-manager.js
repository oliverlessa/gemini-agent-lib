...
const GenerativeAILLM = require('./generative-ai-llm'); // Adicionado import
...
class ChatManager {
    ...
    constructor({ 
        ...
        // Validação condicional do projectId
        if (!llmConfig) {
            throw new Error("ChatManager requer 'llmConfig'.");
        }
        // Só exige projectId se não for GenerativeAILLM (ou se llmType não for especificado, assume Vertex)
        if (llmConfig.llmType !== 'GenerativeAILLM' && !llmConfig.projectId) {
            throw new Error("ChatManager requer 'llmConfig' com 'projectId' para LLMs baseados em Vertex AI.");
        }
        // Validação da apiKey para GenerativeAILLM
        if (llmConfig.llmType === 'GenerativeAILLM' && !llmConfig.apiKey) {
            throw new Error("ChatManager requer 'llmConfig' com 'apiKey' para GenerativeAILLM.");
        }
        ...
        // Instancia o LLM correto com base no llmType
        if (this.baseLlmConfig.llmType === 'GenerativeAILLM') {
            this.sharedLlm = new GenerativeAILLM(this.baseLlmConfig);
            debug.chatManager("Instância compartilhada de GenerativeAILLM criada.");
        } else {
            // Assume VertexAILLM como padrão se llmType não for especificado ou for diferente
            this.sharedLlm = new VertexAILLM(this.baseLlmConfig);
            debug.chatManager("Instância compartilhada de VertexAILLM criada.");
        }
