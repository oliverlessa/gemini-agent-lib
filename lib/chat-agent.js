...
`;
            ...
            debug.chatAgent(`Contexto enriquecido com ${hasFacts ? 'fatos' : ''} ${hasFacts && hasSummary ? 'e' : ''} ${hasSummary ? 'resumo' : ''}`);
        }
    ...
    async processUserMessage(message) {
        ...
        debug.chatAgent(`Histórico atual: ${this.conversationHistory.length} mensagens`);
        ...
        let lastToolResult = null; // Inicializa como null aqui
        ...
        while (response && response.functionCall) {
            ...
            if (functionCall.name === "delegate_task_to_specialist" && this.specialistDelegationEnabled) {
                try {
                    ...
                    const specialistResponse = await specialistAgent.executeTask(); // Pode retornar { text: "..." } ou string
                    ...
                    // Retorna diretamente a resposta do especialista, interrompendo o loop
                    return { text: specialistResponseText };
                    
                } catch (error) {
                    console.error(`[ChatAgent] Erro durante delegação:`, error);
                    // Mantém a lógica de erro, mas a variável functionResult não será usada se o try falhar aqui
                ...
                try {
                    ...
                    // Verifica se o resultado é um sinal de sub-conversa
                    const isSignal = functionResult && functionResult._signal_type;
                    if (isSignal) {
                        debug.chatAgent(`Sinal detectado: ${functionResult._signal_type}`);
                    }
                } catch (error) {
            ...
            lastToolResult = functionResult; // Atualiza o último resultado aqui dentro do loop
        ...
        if (response && !response.functionCall) {
            ...
            // Verifica se a última ferramenta chamada foi uma ferramenta de sinalização
            // e anexa o sinal à resposta final
            const isSignal = lastToolResult && lastToolResult._signal_type; // Usa a variável inicializada/atualizada
            
            if (isSignal) {
                debug.chatAgent("Anexando resultado de sinal à resposta final.");
                // Cria um novo objeto de resposta para não modificar o original do LLM
                const finalResponseObject = {
                    text: response.text, // A resposta textual do LLM
                    _internalToolSignal: lastToolResult // O sinal retornado pela ferramenta
                };
                return finalResponseObject; // Retorna o objeto enriquecido
            }
        }
    ...
    async processFunctionResult(functionName, result) {
        debug.chatAgent(`\nProcessando resultado da função ${functionName}`);
